...
### 제출 문서 QR·공유·생체 잔존 전수 정정
- **무엇**: CTO 요청으로 PLAY_STORE_RELEASE·IOS_SUBMISSION_PREP 전수 확인 → **잔존 4곳 발견·정정**. ①iOS 심사노트 주의문이 아직 "공유·QR·생체로 4.2 방어" ②Play §1 순수웹뷰 정책 근거 ③릴리스 노트 "QR 스캔, 생체인증 앱 잠금" ④제출 전 실기기 체크리스트. + 부록Z·§3.2에 오해 방지 주석.
- **왜**: 방침(웹)에서만 QR을 빼고 신고서·심사노트에 남으면 그것도 불일치. CTO 지적.
- **결과**: 제출용 문서에서 도달불가 기능이 "사용자 기능"으로 남은 것 0(재grep 확인). 릴리스 노트는 "푸시 알림, 링크로 앱 바로 열기"로 교체. 실기기 체크리스트도 도달 가능한 항목만으로 재작성.
- **검증**: grep 전수 + 예외구문 필터링 재확인. 남은 "공유"는 전부 데이터 제3자 공유(Data Safety 용어)로 무관.
- **관련**: 웹 /privacy도 「QR 코드를 스캔할 때」 삭제 반영됨(0c64a67) → 방침·신고서·심사노트 3자 정합.

### 카카오 공유 재확인 — 등록 후에도 실패, "www 유무" 가설 반증
- **무엇**: 오너 도메인 등록 통보 후 bohummap.com / www.bohummap.com **양쪽에서 각각 클릭**(전송은 후킹 차단, URL·error만 포착).
- **왜**: CTO 요청(내 방법으로 재확인). CTO 진단은 "www 유무가 원인".
- **결과**: **양쪽 모두 picker/failed -401 domain mismatched**(caller가 각각 non-www/www로 찍힘). → **www 유무 문제 아님. 카카오가 검사하는 [앱 설정>플랫폼>Web] 사이트 도메인에 우리 도메인이 없다**는 뜻. 대표 도메인·Redirect URI·비즈니스 도메인은 별개 항목이라 거기 넣었을 가능성 유력 → 오너에 등록 위치 재확인 요청.
- **검증**: error 파라미터 base64 디코드로 caller 값 확인(두 케이스 각각). app_key는 JS 키와 일치(다른 앱 아님).
- **관련**: 앱 실기기 판정(스킴 분기·이탈 여부→Android queries 필요성)은 **등록 성공 후에만 가능**하고, 나는 실기기가 없어 오너/CTO 몫.

### 카카오 콘솔 메뉴 경로 정정 (내 안내가 낡았음)
- **무엇**: KAKAO_SHARE_ANALYSIS의 등록 위치 안내를 "앱 설정 > 플랫폼 > Web"(현재 콘솔에 없음) → **"앱 설정 > 제품 링크 관리 > 웹 도메인"**으로 정정. 화면 설명 원문("카카오톡 공유 … 링크 이동을 허용할 웹 도메인") 인용.
- **왜**: CTO가 오너 콘솔 실물 확인 → 메뉴 개편으로 "플랫폼" 항목이 "플랫폼 키"+"제품 링크 관리"로 분리됨. 내 안내대로면 오너가 없는 메뉴를 찾게 됨.
- **결과**: 문서 정정. 남은 가능성 3개(저장 누락/반영 지연/[일반]에 별도 칸) 기록.
- **검증**: 내가 콘솔 접근 불가 → CTO의 화면 실물 관찰을 근거로 채택(내 원 안내는 일반 문서 지식이었고 낡았음).
- **관련**: 내 반증(www·non-www 둘 다 -401, app_key는 JS키 일치)은 유효 — "www 유무" 가설 폐기는 그대로.

### 🔴 제출 게이트 추가 — 스크린샷의 공유 버튼이 실패 상태
- **무엇**: IOS_SUBMISSION_PREP 주의문에 게이트 추가. 콘텐츠 제보(스크린샷 01-home에 「친구에게 보험맵 공유하기」 버튼이 찍혀 있음) + 내 실측(그 웹 공유가 -401로 실패 중)을 결합.
- **왜**: 심사관이 스크린샷을 보고 버튼을 누르면 "요청 실패" 화면을 본다. 제출 전 ①도메인 등록으로 실동작 ②버튼 없는 스크린샷 교체 중 하나 필수.
- **결과**: 게이트 문서화. 콘텐츠 원고의 **"웹 카카오 공유 실동작 확인됨(CTO 클릭 테스트)"는 사실 아님**을 명시(CTO 본인이 "확인 실패지 성공 아님"으로 정정한 건이 잘못 전달됨) → 콘텐츠에 정정 요청.
- **검증**: non-www·www 양쪽 -401 실측(내가 직접), CTO의 "확인 실패" 진술.
- **관련**: 오늘 반복된 패턴(잘못된 검증 메모가 다음 판단을 오도) 세 번째 사례.

### 카카오 도메인 — 공식 문서로 최종 확정(등록처 2곳, -401은 JS SDK 도메인)
- **무엇**: KAKAO_SHARE_ANALYSIS 등록 위치를 표로 확정. ①JavaScript SDK 도메인([플랫폼 키 > JavaScript 키])=**-401 직접 원인, 미등록** ②웹 도메인([제품 링크 관리])=스크랩용, 오너가 이미 등록. 경로 정정 이력 3단계 기록.
- **왜**: CTO가 "내 정정이 틀렸다, 네 원래 안내가 맞다"며 문서 원문 제시 → **내가 직접 카카오 공식 문서(kakaotalk-share/js-link)를 WebFetch로 확인**한 뒤 반영(옮겨 적지 않고 확인).
- **결과**: 오너 조치 대상이 ①로 확정. 앱 키 대조도 완료(콘솔 JS키 = env NEXT_PUBLIC_KAKAO_JS_KEY 일치) → 앱/키 불일치 소거.
- **검증**: 공식 문서 원문 2건("JavaScript 키는 등록된 JavaScript SDK 도메인에서만 사용… 이외 요청은 거절", "스크랩할 웹 페이지의 도메인은 [제품 링크 관리]>[웹 도메인]에 등록돼 있어야").
- **관련**: 교훈 — 메뉴명이 낡았다고 개념까지 폐기하면 안 됨. 내 원 안내(대상)는 맞았고 메뉴명만 낡았음.

### ✅ 카카오 공유 해결 확인 + 모바일 경로 차이 발견
- **무엇**: 오너의 JavaScript SDK 도메인 등록 후 재실측. 데스크톱 UA 양쪽 호스트에서 window.open("sharer.kakao.com/picker/link") = **정상 picker**(이전 /picker/failed -401). SDK도 로드·초기화됨(이전엔 window.Kakao 부재).
- **왜**: CTO 통보 "등록 완료, 즉시 재확인".
- **결과**: **웹 공유 부활 확정.** 원인은 확정대로 JavaScript SDK 도메인 미등록. → 01-home 스크린샷 제출 게이트도 해소 가능(콘텐츠 통보 필요).
...
### 알림톡 조사 → 오너 보류. 꺼낼 조건과 함께 보관
- **무엇**: ALIMTALK_RESEARCH.md — 확정 사실(비용 0으로 심사만 가능·순서·유효기간 1년·정보성 기준) / 재대조 필요(대행사 요금제·단가 미확인·심사 기준·콘솔 화면) / 꺼낼 트리거(문의가 실제로 들어오기 시작할 때) 분리 기록.
- **왜**: 오너 "알림도 일단 보류". CTO 지시 — 폐기하지 말고 재조사 안 하도록 보관.
- **결과**: 구축 0. "심사만 미리"가 비용 0으로 성립함을 확인했으나 보류. 3종 중 "이번 주 N명이 봤어요"는 광고성 위험으로 단독 재설계 권고 기록.
- **검증**: 카카오 비즈니스 가이드 + NHN Cloud 공식 문서. **단가는 공식 문서에 없어 미확인으로 명시**(통념 수치 사용 안 함).
- **관련**: 발송 주체는 서버 → 실행 시 웹 작업, 앱 작업 없음. 「확정」은 확정 시점 사양이라는 규칙 적용.
