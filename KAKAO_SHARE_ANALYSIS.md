...
**가장 유력한 원인: 등록 위치·저장 여부.**
⚠️ **주의 — 콘솔 메뉴가 개편됐다(2026-08-11 CTO가 오너 화면 실물 확인).** 흔히 안내되는 **"앱 설정 > 플랫폼 > Web"은 현재 콘솔에 없다.** 실제 좌측 메뉴(앱 설정 하위)는:
```
일반 / 플랫폼 키 / 어드민 키 / 제품 링크 관리 / 추가 기능 신청 / 카카오톡 채널 / 웹훅 / 멤버 / 고급
```
✅ **웹 도메인 등록 위치 = [앱 설정 > 제품 링크 관리]**
> 화면 설명 원문: 「**카카오톡 공유**, 카카오톡 메시지 등 링크 이동을 허용할 웹 도메인과 네이티브 앱 정보를 관리할 수 있습니다.」
> 웹 도메인: 「링크 이동을 허용할 웹 도메인 정보를 추가하고, 기본 웹 도메인을 선택할 수 있습니다. 최대 10개」

→ **오너 확인 요청**: [앱 설정 > **제품 링크 관리** > 웹 도메인] 목록에 `https://bohummap.com`과 `https://www.bohummap.com`이 **둘 다 실제로 떠 있는지**(추가만 하고 **저장**을 안 눌렀을 수 있음). 스킴 `https://` 포함, 경로 없이 도메인만.
남은 가능성: ① 저장 누락 ② 반영 지연 ③ [앱 설정 > 일반]에 별도 도메인 칸이 또 있을 가능성.
