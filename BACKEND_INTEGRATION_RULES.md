...
| A2 | **소셜 로그인(카카오/구글) 제거됨(2026-08-07, 웹).** 현재 회원가입/로그인은 **웹 폼(이메일 인증) 전용** → 실질적으로 **이메일 인증 회원만 존재**. 앱은 로그인 UI를 만들지 않고 웹 폼에 위임. 앱의 OAuth 우회 인프라(Custom Tab+스킴 콜백)는 **유지**(A-001 정정) — Google은 미발화이나 카카오 OAuth 도입 시 재사용. | `is_full_member` = `provider='email' AND email_verified_at IS NOT NULL AND approved`. (과거 카카오/구글 "탐색 전용"은 소셜 제거로 해소) |
