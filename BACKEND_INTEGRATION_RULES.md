...
| A2 | **로그인 정책(2026-08-07)**: Google 로그인은 웹에서 제거됨. **카카오 로그인 도입 확정 — 카카오 사용자는 정식 회원**(과거 "탐색 전용" 서술은 폐기). 현재는 웹 카카오 구현 전이라 이메일 인증 회원 위주이나, 카카오 도입 후엔 카카오도 정식 회원. 앱은 로그인 UI를 만들지 않고 **웹 폼/웹 OAuth에 위임**. OAuth 우회 인프라(Custom Tab+스킴 콜백)는 **유지**(A-001) — 카카오가 재사용. 조사: [KAKAO_LOGIN_RESEARCH.md](KAKAO_LOGIN_RESEARCH.md). | 회원 판정은 `is_full_member()`(웹). 소셜 회원 포함 여부는 웹 구현 기준을 따름 |
