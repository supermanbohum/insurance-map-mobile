...
| **로그인: 웹 폼(이메일 인증) 전용. Google 제거로 OAuth 미발화이나 우회 인프라는 유지(A-001 정정, 카카오 재사용 대비)** | ✅ |
...
| 간편로그인/OAuth | **A-001 정정(2026-08-07)**: Google은 웹에서 제거돼 미발화이나, **OAuth 우회 인프라(Custom Tab+스킴 콜백)는 유지** — 카카오 OAuth 도입 예정이라 재사용. A-004로 카카오 옵션 조사 중 | 앱 |
| **카카오 로그인 도입 검토(A-004)** | Expo57에서 카카오 로그인 옵션 비교·권고 조사(구현 X). 카카오 확정 시 콘솔 등록값(패키지명/키해시/리다이렉트URI) 필요 | 앱+오너 |
